...
@section('content')
    ...
    <div class="card" style="margin-bottom: 20px;">
        ...
        <div class="card-body">
            @if($riskAssessments->isEmpty())
                <div class="empty-state" style="padding: 24px 20px;">
                    ...
                    @if($bebasLabRequests->isEmpty())
                        ...
                        <a href="{{ route('mahasiswa.risk-assessment.index') }}" class="btn btn-danger" style="margin-top: 12px; display: inline-flex;">
                            Ajukan Risk Assessment
                        </a>
                    @else
            ...
            @else
                <form action="{{ route('mahasiswa.bebas-lab.store') }}" method="POST" style="max-width: 540px; text-align: left;">
                    @csrf
    ...
    {{-- Riwayat Bebas Lab Section --}}
    @if($historyRequests->isNotEmpty())
    <div class="card" style="margin-bottom: 20px;">
        <div class="card-header">
            <h2>Riwayat Bebas Lab</h2>
        </div>
                    ...
                    <tbody>
                        @foreach($historyRequests as $history)
                            <tr>
                                ...
                                <td>
                                    <strong>{{ $history->riskAssessment?->id_ra ?? $history->risk_assessment_id ?? '-' }}</strong>
                                    @if($history->periode > 1)
                                        <span class="badge badge-warning" style="font-size: 11px; padding: 2px 6px;">(Periode {{ $history->periode }})</span>
                                    @endif
                                    <br>
                                    <span style="color: #6b7280; font-size: 12px;">{{ $history->riskAssessment?->topik_judul ?? '-' }}</span>
                                </td>
                                <td>{{ $history->riskAssessment?->daftarLab?->Nama_Laboratorium ?? '-' }}</td>
                                <td>
                                    @php 
                                        $statusBadges = [
                                        ...
                                        $statusBadge = $statusBadges[$history->status] ?? 'badge-warning';
                                    @endphp
                                    <span class="badge {{ $statusBadge }}">{{ ucfirst($history->status) }}</span>
                                </td>
                                <td>
                                    <span class="badge {{ $history->is_active ? 'badge-success' : 'badge-secondary' }}">
                                        {{ $history->is_active ? 'Aktif' : 'Tidak Aktif' }}
                                    </span>
                                </td>
                                <td>
                                    <div style="display: flex; gap: 6px; flex-wrap: wrap;">
                                        <a href="{{ route('mahasiswa.bebas-lab.show', $history->id) }}" class="btn btn-sm btn-primary">
                                            Lihat Detail
                                        </a>
                    ...
                    <table class="data-table">
                        ...
                        <tbody>
                            @foreach($peminjamanAlats as $alat)
                                <tr>
                                    <td>{{ $alat->riskAssessment?->daftarLab?->Nama_Laboratorium ?? '-' }}</td>
